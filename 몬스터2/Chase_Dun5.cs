...
public class Chase_Dun5 : MonoBehaviour {
	...
	IEnumerator Chasing()
	{
		while (_FSM._state == FSM_DUN5.STATE.CHASE) {
			...
			if (Vector3.Distance (_player.position, this.transform.position) < 10 && (angle < 30  || angle >330 || pursuing)) 
			{
				...
				if (direction.magnitude > 5) {
					...
					A.Stop ();
					_FSM._state = FSM_DUN5.STATE.ATTACK;
				}
			}
			else{
				pursuing = false;
				_FSM._state = FSM_DUN5.STATE.MOVE;
			}
